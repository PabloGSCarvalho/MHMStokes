...
void DarcyPTest::F_source(const TPZVec<REAL> &x, TPZVec<STATE> &f){
...
TPZCompMesh *DarcyPTest::CMesh_v(TPZGeoMesh *gmesh, int Space, int pOrder)
{
    ...
    if (Space==1) {
        ...
        cmesh->SetAllCreateFunctionsContinuous(); //Criando funções H1 (descontínuo):
...
TPZCompMesh *DarcyPTest::CMesh_m(TPZGeoMesh *gmesh, int Space, int pOrder, STATE visco, STATE permeability, STATE theta)
{
    ...
    auto BCond0 = material->CreateBC(material, fmatBCbott, fneumann, val1, val2);
    BCond0->SetForcingFunctionBC(Sol_exact, bc_inte_order);
    cmesh->InsertMaterialObject(BCond0);

    auto BCond1 = material->CreateBC(material, fmatBCtop, fneumann, val1, val2);
    BCond1->SetForcingFunctionBC(Sol_exact, bc_inte_order);
    cmesh->InsertMaterialObject(BCond1);

    auto BCond2 = material->CreateBC(material, fmatBCleft, fneumann, val1, val2);
    BCond2->SetForcingFunctionBC(Sol_exact, bc_inte_order);
    cmesh->InsertMaterialObject(BCond2);

    auto BCond3 = material->CreateBC(material, fmatBCright, fneumann, val1, val2);
