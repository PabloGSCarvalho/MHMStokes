...
TPZMHMBrinkmanMaterial::TPZMHMBrinkmanMaterial() : TPZMatWithMem<TPZFMatrix<STATE>, TPZDiscontinuousGalerkin >(){
    //fDim = 1;
...
TPZMHMBrinkmanMaterial::TPZMHMBrinkmanMaterial(int matid, int dimension, int space, STATE viscosity, STATE theta, STATE Sigma) : TPZMatWithMem<TPZFMatrix<STATE>, TPZDiscontinuousGalerkin >(matid),fDimension(dimension),fSpace(space),fViscosity(viscosity),fTheta(theta),fSigma(Sigma)
{
    // symmetric version
    //fTheta = -1;
...
TPZMHMBrinkmanMaterial::TPZMHMBrinkmanMaterial(const TPZMHMBrinkmanMaterial &mat) : TPZMatWithMem<TPZFMatrix<STATE>, TPZDiscontinuousGalerkin >(mat),fDimension(mat.fDimension),fSpace(mat.fSpace), fViscosity(mat.fViscosity), fTheta(mat.fTheta), fSigma(mat.fSigma)
{
    fk= mat.fk;
...
TPZMHMBrinkmanMaterial::~TPZMHMBrinkmanMaterial(){
...
void TPZMHMBrinkmanMaterial::FillDataRequirements(TPZVec<TPZMaterialData> &datavec)
{
...
void TPZMHMBrinkmanMaterial::FillBoundaryConditionDataRequirement(int type,TPZVec<TPZMaterialData> &datavec)
{
    int ndata = datavec.size();
    for (int idata=0; idata < ndata ; idata++) {
        datavec[idata].SetAllRequirements(false);
...
void TPZMHMBrinkmanMaterial::FillDataRequirementsInterface(TPZMaterialData &data)
{
...
void TPZMHMBrinkmanMaterial::Print(std::ostream &out) {
...
int TPZMHMBrinkmanMaterial::VariableIndex(const std::string &name) {
...
int TPZMHMBrinkmanMaterial::NSolutionVariables(int var) {
...
void TPZMHMBrinkmanMaterial::Solution(TPZVec<TPZMaterialData> &datavec, int var, TPZVec<STATE> &Solout) {
...
void TPZMHMBrinkmanMaterial::ComputeDivergenceOnMaster(TPZVec<TPZMaterialData> &datavec, TPZFMatrix<STATE> &DivergenceofPhi)
...
void TPZMHMBrinkmanMaterial::Write(TPZStream &buf, int withclassid) const{
...
void TPZMHMBrinkmanMaterial::Read(TPZStream &buf, void *context) {
...
void TPZMHMBrinkmanMaterial::FillGradPhi(TPZMaterialData &dataV, TPZVec< TPZFMatrix<REAL> > &GradPhi){
...
void TPZMHMBrinkmanMaterial::Contribute(TPZVec<TPZMaterialData> &datavec, REAL weight, TPZFMatrix<STATE> &ek, TPZFMatrix<STATE> &ef){
...
TVar TPZMHMBrinkmanMaterial::Inner(TPZFMatrix<TVar> &S, TPZFMatrix<TVar> &T){
...
////////////////////////////////////////////////////////////////////
STATE TPZMHMBrinkmanMaterial::InnerVec(TPZFMatrix<STATE> &S, TPZFMatrix<STATE> &T){
...
STATE TPZMHMBrinkmanMaterial::Tr( TPZFMatrix<REAL> &GradU ){
...
void TPZMHMBrinkmanMaterial::FillVecShapeIndex(TPZMaterialData &data)
{
...
void TPZMHMBrinkmanMaterial::Errors(TPZVec<TPZMaterialData> &data, TPZVec<STATE> &u_exact, TPZFMatrix<STATE> &du_exact, TPZVec<REAL> &errors)
{
